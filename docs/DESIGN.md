...
| D9 | Sequence conflicts | `Sequence` is a diff3 over element keys aligned by a longest common subsequence (found as D18 says); both-inserted blocks at one position are kept in key order and flagged; two changes to one stretch conflict and the base stretch stays in the value, so the result is a merge only when `Clean()` | Deterministic, side-neutral, and never invents an order the writers did not |
