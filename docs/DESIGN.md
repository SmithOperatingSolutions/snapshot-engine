...
| D13 | Transactions | Snapshot isolation on the working set as of `Begin`; writes buffered per object and flushed before a read (read-your-writes); `Commit` swaps its namespace in when the working set has not moved, else three-way merges base, ours and the current namespace through the core's merge and swaps that in, any conflict `ErrSerialization` with nothing written; a lost swap is retried up to five times. `Staged` follows `Working`: there is no staging area at the engine | The Engine Spec's L4 transaction rules; a merge decides what two writers meant, the models' policies apply per cell, key and field |
