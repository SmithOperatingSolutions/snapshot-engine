...
| D3 | Model ids | Table is 3, as the Storage Core Spec's registry table says. kv is proposed as 4 and document as 5; the core's table is the registry of record and is amended there before either model ships | One registry across every consumer of the core; two plugins can never collide |
