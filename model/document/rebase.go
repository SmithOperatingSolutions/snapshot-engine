...
// Rebase applies to onto what side changed from base, record by record, and
// is Merge(base, onto, side) whenever that merge finds no record changed on
// both sides. It reads side's changes and one record of onto per change, not
// what onto changed: its cost is the side's, however far onto has moved
// from base. A record side changed that onto holds otherwise than base did
