...
// Batch applies many sides to one collection with one flush (#22). Stub:
// it takes every side and writes nothing.
type Batch struct {
	s    mapobject.Spec
	onto *prolly.Map
}
...
// Check reads what side changed from base and checks each record against
// the target and the sides taken so far.
