...
// Batch applies many sides to one target with one flush (#22): each side
...
type Batch struct {
	...
	onto  *prolly.Map
	ed    *prolly.Editor
	taken map[string]bool // keys the sides taken so far changed
}

// Pending is one side's changes, checked against the batch and not yet
// taken: taking it makes its keys the batch's, refusing later sides that
// change them.
type Pending struct {
	edits []edit
}
...
// target and the sides taken so far: a key the target holds otherwise than
// base did, or one a side taken changed, is ErrChangedSince, and the side
// is not taken. It reads the side's changes and one key of the target per
